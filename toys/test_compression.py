...
for f in sys.argv[1:]:
    ...
    if original_size == 0:
        ...
        continue
    print("Benchmarking file:", f, 'size:', '%sB' % human(original_size), '(%d)' % original_size)
    for name, comp, out, decompress in compressors:
        if os.system('which %s > /dev/null' % name) == 0:
            ...
            print('    Size after: %5sB (%d), %4.1f%% size (factor %5.2f).' % (
                human(compressed), compressed, 100.0 * compressed / original_size, float(original_size) / compressed
            ))
            print('    Compressed in %5.3f s, in: %8sB/s, out %8sB/s.' % (
                elapsed, human(original_size / elapsed), human(compressed / elapsed)))
            elapsed_2 = timed(decompress % dict(f=f))
            print('    Decompress in %5.3f s, in: %8sB/s, out %8sB/s.' % (
                elapsed_2, human(compressed / elapsed_2), human(original_size / elapsed_2)))
            print('  ' + '-' * 40)
    print('=' * 60)
