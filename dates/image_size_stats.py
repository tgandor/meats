...
PNG = ".png"
PNGS = {PNG}
SUPPORTED_IMAGE_EXTENSIONS = JPEGS | PNGS
...
def image_matches(filename, requested_extension=""):
    lower_name = filename.lower()
    if requested_extension:
        return lower_name.endswith(normalize_extension(requested_extension))
    return any(lower_name.endswith(ext) for ext in SUPPORTED_IMAGE_EXTENSIONS)
...
def main():
    ...
    parser.add_argument(
        ...
        "-x",
        default="",
        help='Extension to process; empty means all supported images (jpg, jpeg, png)',
    )
    parser.add_argument("--verbose", "-v", action="store_true")
    args = parser.parse_args()

    # Keep an explicit empty default in the CLI and allow a requested filter to be
    # dotted or dottedless, while the default scan means "all supported images".
    ...
    for path, directories, files in os.walk(args.directory):
        for basename in files:
            ...
            if image_matches(basename, args.extension):
