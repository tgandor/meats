...
def _filename2(fileinfo):
    ...
    try:
        ...
        sys.stderr.write(
            "Error decoding {}, using {} instead\n".format(
                repr(fileinfo.filename), gelded
            )
        )
        return gelded
...
def _print_short(fi):
    try:
        print(
            "%9d  %d-%02d-%02d %02d:%02d   %s"
            % ((fi.file_size,) + fi.date_time[:-1] + (_filename(fi),))
        )
    except:
        print(
            "%9d  %d-%02d-%02d %02d:%02d   %s(ERR)"
...
def _extract(fi, z):
    ...
    if name.startswith("/") or name.startswith("../") or name.find("/../") != -1:
        sys.stderr.write("Malicious path found: %s, not extracting.\n" % name)
        return
    if options["-d"] != ".":
