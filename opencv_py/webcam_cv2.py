...
parser.add_argument(
    "--delay", "-s", type=float, help="Time to sleep between frames (seconds)"
)
parser.add_argument(
    "--frame", type=int, help="Starting frame to (try to) skip to.", default=0
)
parser.add_argument(
    "device",
    type=str,
    nargs="?",
    help="device number, video file name or network stream URL",
    default="0",
)
...
print(
    "FourCC:",
    (
        [chr(((initial_fourcc >> (8 * i)) & 0xFF)) for i in range(4)]
        if initial_fourcc
        else "0"
    ),
)
...
if args.fourcc:
    cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*args.fourcc))  # type: ignore
...
while True:
    ...
    if record:
        ...
        print(
            "frame saved: {}, {:,} KB, {:,} MB total, ~ {:,} MB/h".format(
    ...
    """
        ...
        print(
            "Unhandled key: {k}, 0x{k:x}, {char}, raw: {r}".format(
                k=key, r=raw_key, char=repr(chr(key))
            )
        )
